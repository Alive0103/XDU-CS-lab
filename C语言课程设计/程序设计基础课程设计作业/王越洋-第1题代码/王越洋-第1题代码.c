...
void add_entry(AddressBook* address_book);// 添加通讯录条目
void delete_entry(AddressBook* address_book);// 删除通讯录条目
void show_all_entries(AddressBook* address_book);// 显示所有通讯录条目
void find_entry(AddressBook* address_book) ;// 查找通讯录条目
void save_address_book(AddressBook* address_book, const char* filename) ;// 保存通讯录到文件
void load_address_book(AddressBook* address_book, const char* filename) ; // 从文件加载通讯录
...
void delete_entry(AddressBook* address_book) {
    ...
    address_book->num_entries--;//对应的计数也要减1
    printf("删除成功。\n");
}
